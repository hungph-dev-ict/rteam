...
/**
 * Safe CSS ID escaping for use in attribute selectors
 * Escapes special characters that would break CSS selectors
 */
function cssEscape(id) {
  return id.replace(/[!"#$%&'()*+,./:;<=>?@[\\\]^`{|}~\s]/g, '\\$&');
}

function buildRadioId(key) {
  // Keys with underscore: "Japanese_Listening" → "Japanese_Listening_choose"
  // Keys without underscore: "English" → "English__choose"
  if (key.includes('_')) {
    return `${key}_choose`;
  } else {
    return `${key}__choose`;
    ...
    for (const [key, value] of Object.entries(payload)) {
      try {
        ...
        if (TEXT_INPUT_KEYS.includes(key)) {
          // Use attribute selector for safety with special characters in IDs
        ...
        if (typeof value === 'object' && value !== null) {
          ...
          // Use attribute selector with escaped ID for safety
          if (radioValue !== undefined && radioValue !== null) {
            const radio = page.locator(`[id="${cssEscape(radioGroupId)}"] input[value="${radioValue}"]`);
            await radio.click();
            results.filled++;
          }
